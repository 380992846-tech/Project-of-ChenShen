...
#### 受限环境下的满载稳态观测（AutoDL A800）
容器不能锁频时，改用**矩阵规模**作为变量（`burn_in.py --levels 8192 12288 16384`），FP16 打满张量核，
采集满载稳态功耗。频率恒定在最高档 1410 MHz，但功耗随算力规模**单调上升**——验证负载与热设计功耗（TDP）的物理相关性：

| 矩阵规模 | 频率 | 满载功耗 |
|---|---|---|
| 8192 | 1410 MHz | 69.1 W |
| 12288 | 1410 MHz | 74.4 W |
| 16384 | 1410 MHz | 79.8 W |

![GEAR · A800 烧机：矩阵规模 → 功耗（正相关）](docs/images/burnin_power.png)
