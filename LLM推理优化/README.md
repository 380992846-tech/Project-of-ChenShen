...
| **量化** | FP32 vs INT8/INT4/FP8 | 存储压缩 4–5×，PPL 均 ≈414 不变；但 **CPU 动态 INT8 反而更慢**（3.53 vs 1.07 ms/tok），真正加速需 weight-only + 融合算子 |
| **投机解码** | γ=4, 96 token | target 前向 1.0 → **0.20 次/token**（≈5× 算法层）；但 **墙钟仅 0.95×**（CPU 小模型 draft 不够快，需真 GPU / 更快 draft 兑现） |
