...
$$
M_{\text{load}} \approx N \times b \times 1.2
$$
...
$$
M_{\text{total}} \approx N \times b \times 1.2 + 4 \times \text{batch} \times \text{seq\_len} \times \text{hidden\_dim} \times \text{num\_layers}
$$
...
$$
C_{\text{all-reduce}} = 2\psi \cdot \frac{G-1}{G}
$$
...
$$
T_{\text{MinHash}} = O(N \cdot k \cdot \log N), \quad S_{\text{MinHash}} = O(N \cdot k)
$$
...
$$
W = W_0 + BA, \quad \text{其中 } B \in \mathbb{R}^{d \times r},\ A \in \mathbb{R}^{r \times k},\ r \ll \min(d, k)
$$
...
$$
\mathbb{E}\bigl[ \| \nabla L(\theta_T) \|^2 \bigr] \le O\!\left(\frac{1}{\sqrt{T}}\right) + O\!\left(\frac{C}{T}\right)
$$
...
$$
C_{\text{HBM}} = O\!\left(\frac{N^2 d}{M}\right)
$$
...
$$
\| w - w' \|_\infty \le \frac{\max - \min}{2^{b+1}}
$$
...
$$
\delta = -\frac{w_i - q_i}{(H^{-1})_{ii}} \cdot (H^{-1})_{:,\ i}
$$
...
$$
M_{\text{total}} = M_{\text{param}} + M_{\text{KV cache}} + M_{\text{activations}}
$$
...
$$
S = \frac{1}{\dfrac{1}{K} + \dfrac{\alpha}{1 - (1-\alpha)^K} \cdot \dfrac{t_v}{t_d}}
$$
...
$$
\text{Cost}_{\text{per 1M}} = \frac{\text{GPU 每小时租赁价格} \times \text{生成 1M Token 所需时间}}{\text{GPU 利用率}}
$$
...
$$
\text{Forgetting} = \frac{\text{Acc}_{\text{old}} - \text{Acc}_{\text{new}}}{\text{Acc}_{\text{old}}} \times 100\%
$$
...
$$
\mathcal{L}_{\text{aux}} = \alpha \cdot \text{CV}(\text{专家利用率})
$$
...
$$
H(\mathbf{a}_t) = -\sum_{i=1}^{t} a_{ti} \log a_{ti}
$$
